...
use sciport_rs::signal::{band_filter::BandFilter, bessel::*, output_type::Zpk, Analog};

#[test]
fn with_py_test_bessel() {
    for _ in 0..500 {
        let order = rand::thread_rng().gen_range(0..80);
...
fn test_besselap() {
    for i in 1..80 {
        ...
        let rust = besselap(i, BesselNorm::Phase);
...
fn test_bessel(order: u32, band_filter: BandFilter, analog: Analog) {
    ...
    let rust = BesselFilterStandalone::<Zpk>::filter(order, band_filter, analog, BesselNorm::Phase);
