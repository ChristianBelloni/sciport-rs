...
fn root_scalar() {
    ...
    for bracket_method in methods {
        ...
        let res = solve_from_bracket(fun, &bracket_method, bracket, criteria.clone());
        println!("{}", res);
    }

    print_divider("Secant".to_string());
    let res = secant_method(fun, x0, x1, criteria.clone());
    println!("{}", res);
    ...
    let res = fixed_point_method(fun, x0, criteria.clone());
    println!("{}", res);
